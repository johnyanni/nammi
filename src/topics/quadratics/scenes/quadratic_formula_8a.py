...
class QuadraticFormula(MathTutorialScene):
    def construct(self):
        ...
        # Look for (a) occurrences
        step_1_a_indices = []
        for i in range(len(step_1_with_vars[0])):
            if i+2 < len(step_1_with_vars[0]) and "(" in step_1_with_vars[0][i].tex_string and "a" in step_1_with_vars[0][i+1].tex_string and ")" in step_1_with_vars[0][i+2].tex_string:
                step_1_a_indices.append(i+1)  # Index of 'a'
        
        # Look for (b) occurrences
        step_1_b_indices = []
        for i in range(len(step_1_with_vars[0])):
            if i+2 < len(step_1_with_vars[0]) and "(" in step_1_with_vars[0][i].tex_string and "b" in step_1_with_vars[0][i+1].tex_string and ")" in step_1_with_vars[0][i+2].tex_string:
                step_1_b_indices.append(i+1)  # Index of 'b'
                
        # Look for (c) occurrence
        step_1_c_indices = []
        for i in range(len(step_1_with_vars[0])):
            if i+2 < len(step_1_with_vars[0]) and "(" in step_1_with_vars[0][i].tex_string and "c" in step_1_with_vars[0][i+1].tex_string and ")" in step_1_with_vars[0][i+2].tex_string:
                step_1_c_indices.append(i+1)  # Index of 'c'
        
        # Get the mobjects for variables
        step_1_a_parts = [step_1_with_vars[0][i] for i in step_1_a_indices]
        step_1_b_parts = [step_1_with_vars[0][i] for i in step_1_b_indices]
        step_1_c_part = step_1_with_vars[0][step_1_c_indices[0]] if step_1_c_indices else None
        ...
        # Create a copy with values for later use
        ...
        with self.voiceover(
            ...
            # Replace b with 11 in the first occurrence
            self.wait_until_bookmark("negative_b")
            if step_1_b_parts and len(step_1_b_parts) > 0:
            ...
            # Replace b with 11 in the second occurrence (for b^2)
            if step_1_b_parts and len(step_1_b_parts) > 1:
            ...
            # Replace a with 1 in the first occurrence
            if step_1_a_parts and len(step_1_a_parts) > 0:
                self.play(ReplacementTransform(a_value.copy(), step_1_a_parts[0]), run_time=1)

            self.wait_until_bookmark("c")
            # Replace c with 20
            if step_1_c_part:
                self.play(ReplacementTransform(c_value.copy(), step_1_c_part), run_time=1)
            ...
            # Replace a with 1 in the second occurrence
            if step_1_a_parts and len(step_1_a_parts) > 1:
                self.play(ReplacementTransform(a_value.copy(), step_1_a_parts[1]), run_time=1)
            
            # After all individual replacements, do a final transform to the version with all values
            self.play(
