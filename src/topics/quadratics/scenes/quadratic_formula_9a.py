...
class QuadraticFormula(MathTutorialScene):
    def construct(self):
        ...
        step_1_eq = MathTex(r"x = \frac{-(b) \pm \sqrt{(b)^2 - 4(a)(c)}}{2(a)}").scale(TEX_SCALE)
        
        # Apply smart colorization to step 1
        self.apply_smart_colorize(
            step_1_eq,
            {
                "a": A_COLOR,
        ...
        # Instead of searching for tex_string, use direct search for the variable symbols
        # Find a, b, c characters using search_shape_in_text
        step_1_a_indices = search_shapes_in_text(step_1_eq, [MathTex("a")])
        step_1_b_indices = search_shapes_in_text(step_1_eq, [MathTex("b")])
        step_1_c_indices = search_shapes_in_text(step_1_eq, [MathTex("c")])
        
        # Get the mobjects for variables
        step_1_a_parts = [step_1_eq[0][i] for i in step_1_a_indices]
        step_1_b_parts = [step_1_eq[0][i] for i in step_1_b_indices]
        step_1_c_parts = [step_1_eq[0][i] for i in step_1_c_indices]
        ...
        # Create a copy with values for the final result
        ...
        step_1_with_values.move_to(step_1_eq)
        ...
        with self.voiceover(
            ...
            self.wait_until_bookmark("c")
            # Replace c with 20
            if step_1_c_parts and len(step_1_c_parts) > 0:
                self.play(ReplacementTransform(c_value.copy(), step_1_c_parts[0]), run_time=1)
