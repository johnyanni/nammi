...
TEMPLATE_PARAMS = {
    "equation": {
        ...
        "spoken": "y equals negative four x minus three",  # Spoken version for voiceover
        "slope_indices": [1, 2],          # Source indices in problem_text_equation for slope
        "y_intercept_indices": [3, 4],    # Source indices in problem_text_equation for y-intercept
        "step1_info2_value_indices": [-2, None],  # Target indices in step1_info_2 for slope value
        "step1_info3_value_indices": [-2, None]   # Target indices in step1_info_3 for y-intercept value
    },
    "points": {
...
class LinearEquationsGraphLinearEquation(MathTutorialScene):
    ...
    def construct(self):
        ...
        step3_info_1 = MathTex(f"\\text{{Slope }} = {slope_display} = \\frac{{\\text{{rise}}}}{{\\text{{run}}}} = {slope_fraction}").scale(MATH_SCALE)
        ...
        with self.voiceover(f"The run is {run_spoken} unit {run_direction.lower()}. <bookmark mark='run'/> {direction_explanation}"):
            scroll_mgr.prepare_next(self)
        self.wait(STANDARD_PAUSE)
