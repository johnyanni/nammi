...
TEMPLATE_PARAMS = {
    ...
    "equation": {
        "slope": -3/4,                     # The slope value as a number (-3/4)
        "y_intercept": 2,                  # The y-intercept value as a number
        "formatted": "y=2-\\frac{3}{4}x",  # Formatted equation with LaTeX (in y=b+mx form)
    },
    ...
    "display": {
        ...
        "spoken_equation": "y equals two minus three-fourths x",  # Spoken version for voiceover
    },
    
    # Animation indices for transformations
    "indices": {
        "slope_src_indices": [-5, -1],         # Source indices in problem_text_equation for slope
        "y_intercept_src_indices": [2, 3],   # Source indices in problem_text_equation for y-intercept
    ...
    "geometry": {
        ...
        "second_point": [4, -1],           # Coordinates of second point
        "axes_range": [-6, 6, 1],          # Default range for axes: [min, max, step]
        "x_line_range": [-5, 5]            # X-range for plotting the line (within -5 to 5)
    },
    
    # Rise and run values for slope visualization
    "rise_run": {
        "rise_value": 3,                  # Absolute value of rise
        ...
        "rise_direction": "DOWN",         # Direction for rise: DOWN (because negative slope)
        "run_direction": "RIGHT"          # Direction for run: RIGHT
    },
