...
class LinearEquationsGraphLinearEquation(MathTutorialScene):
    ...
    def construct(self):
       ...
       # Create rise arrows based on parameters
        ...
        # Either define these in TEMPLATE_PARAMS or calculate them here
        rise_for_fraction = rise_value  # Should be 1
        run_for_fraction = run_value    # Should be 4

        # Format the slope fraction based on sign and value
        if slope < 0:
            slope_fraction = f"-\\frac{{{rise_for_fraction}}}{{{run_for_fraction}}}"
        else:
            slope_fraction = f"\\frac{{{rise_for_fraction}}}{{{run_for_fraction}}}"
        ...
        SmartColorizeStatic(step3_info_1,
            {r"\text{rise}": rise_color, r"\text{run}": run_color, 
             f"{rise_for_fraction}": rise_color, f"{run_for_fraction}": run_color,
