...
class QuadraticFormula02a(MathTutorialScene):
    def construct(self):
        ...
        with self.voiceover(
            text=f"""Negative {B_VALUE} stays as negative {B_VALUE}. 
            {B_VALUE} squared is {B_SQUARED}. 4. times. {A_VALUE} times {C_VALUE} is {FOUR_AC}. And 2 times {A_VALUE} is {TWO_A}."""
        ) as tracker:
        ...
        with self.voiceover(
            text=f"""{B_SQUARED} minus {FOUR_AC} equals {DISCRIMINANT}."""
        ) as tracker:
        ...
        with self.voiceover(
            text=f"""We can factor {DISCRIMINANT} as 16 times 3."""
        ) as tracker:
        ...
        with self.voiceover(
            text=f"""Negative {B_VALUE} divided by {TWO_A} is negative 5."""
        ) as tracker:
        ...
        with self.voiceover(
            text=f"""And 4 square root 3 divided by {TWO_A} is 2 square root 3."""
        ) as tracker:
            scroll.transform_from_copy(
