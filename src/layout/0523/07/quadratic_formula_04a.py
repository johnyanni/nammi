...
class QuadraticFormula04a(MathTutorialScene):
    def construct(self):
        # ============================================
        # CONFIGURATION
        # ============================================
        ...
        NEG_B = -B_VALUE  # This is -b in the formula
        ...
        # ============================================
        # SECTION 3: REARRANGE TO STANDARD FORM
        # ============================================
        rearrange_intro = Tex("First, get the equation in standard form:").scale(LABEL_SCALE)
        rearrange_label = Tex("Add 9 to both sides").scale(LABEL_SCALE)
        rearrange_annotation = self.create_annotated_equation(
            r"-x^2 - 3x = -9",
            r"+9",
            r"-3x",
            "-9"
        )
        standard_result = MathTex(rf"-x^2 - 3x + 9 = 0").scale(MATH_SCALE)
        
        rearrange_group = VGroup(
            rearrange_intro,
            rearrange_label,
            rearrange_annotation,
            standard_result
        ).arrange(DOWN, aligned_edge=LEFT, buff=0.2)
        
        # ============================================
        # SECTION 4: IDENTIFY COEFFICIENTS
        # ============================================
        identify_label = Tex("Now, identify the coefficients:").scale(LABEL_SCALE)
        identify_equation = MathTex(rf"-x^2 - 3x + 9 = 0").scale(MATH_SCALE)
        ...
        coefficient_values_in_equation = self.parse_elements(identify_equation,
            ('a_value', '-x', 0, A_COLOR),  # a = -1 (the minus sign)
            ('b_value', '-3', 0, B_COLOR),
            ('c_value', '9', 0, C_COLOR)
        )
        ...
        coefficient_values_in_formula = self.parse_elements(substitute_equation,
            ...
            ('a_in_4ac', str(A_VALUE), 0, A_COLOR, 0),
        ...
        sqrt_intermediate = MathTex(r"\sqrt{45} = \sqrt{9 \times 5} = 3\sqrt{5}").scale(M_MATH_SCALE).set_color(LIGHT_GRAY)
        sqrt_result = MathTex(rf"x = \frac{{3 \pm 3\sqrt{{5}}}}{{-2}}").scale(MATH_SCALE)
        ...
        SQRT_5 = sqrt(5)
        SOLUTION1_DECIMAL = -(3 + 3*SQRT_5) / 2
        SOLUTION2_DECIMAL = -(3 - 3*SQRT_5) / 2
        
        # Answer 1
        answer1_label = Tex("Solve for $x$:").scale(LABEL_SCALE)
        answer1_exact = MathTex(rf"x = \frac{{3 + 3\sqrt{{5}}}}{{-2}}").scale(MATH_SCALE)
        ...
        answer2_exact = MathTex(rf"x = \frac{{3 - 3\sqrt{{5}}}}{{-2}}").scale(MATH_SCALE)
        ...
        sol_steps_elements = VGroup(
            ...
            *simplify2_step,
            ...
            sqrt_label,
            sqrt_parsing['sqrt_45'],
            sqrt_parsing['sqrt_45_solve'],
            sqrt_parsing['sqrt_45_solved'],
            sqrt_result
        )
        ...
        tip_1 = QuickTip(r"When $a$ is negative, be careful with signs throughout the calculation")
        tip_1.to_corner(DL)
        ...
        with self.voiceover(
            ...
            We have negative x squared minus three x equals negative nine."""
        ) as tracker:
        ...
        with self.voiceover(
            text="""Let's add nine to both sides."""
        ) as tracker:
            scroll.prepare_next(run_time=2)  # Show label
            
        with self.voiceover(
            text="""We add nine to negative nine to get zero.
            This gives us negative x squared minus three x plus nine equals zero.
            <bookmark mark="A"/>
        ...
        with self.voiceover(
            text="""Now we identify the coefficients. Notice that a is negative."""
        ) as tracker:
        ...
        with self.voiceover(
            text="""In our equation, the coefficient of x squared is negative one."""
        ) as tracker:
        ...
        with self.voiceover(
            text="""The coefficient of x is negative three."""
        ) as tracker:
        ...
        with self.voiceover(
            text="""And the constant term is nine."""
        ) as tracker:
        ...
        with self.voiceover(
            text="""We replace b with negative three, appearing twice in the formula."""
        ) as tracker:
        ...
        with self.voiceover(
            text="""We replace a with negative one in the four. ac term."""
        ) as tracker:
        ...
        with self.voiceover(
            text="""And a with negative one again in the denominator."""
        ) as tracker:
            self.play(self.indicate(coefficient_values['a_value'], color=None, run_time=2))
        ...
        with self.voiceover(
            text="""We have negative negative three, which becomes positive three.
            Negative three squared is nine. 
            Four. times negative one times nine is negative thirty-six. 
            And two times negative one is negative two."""
        ) as tracker:
        ...
        self.wait(1)
        scroll.scroll_down(steps=8, run_time=1)
        ...
        with self.voiceover(
            text="""Nine minus negative thirty-six is nine plus thirty-six, which equals forty-five."""
        ) as tracker:
        ...
        with self.voiceover(
            text="""Since the square root of nine is three, 
            we get three times the square root of five."""
        ) as tracker:
            scroll.prepare_next(run_time=2)  # Show 3√5
            scroll.prepare_next(run_time=3)  # Show final formula with 3√5
        
        self.wait(2)
        
        # === STEP 8: Final answers ===
        with self.voiceover(
            text="""This gives us two real solutions."""
        ) as tracker:
            self.play(
        ...
        with self.voiceover(
            text="""For the first solution, we add: 
            three plus three times the square root of five, all divided by negative two."""
        ) as tracker:
            self.play(
                TransformFromCopy(sqrt_result, answer1_exact, run_time=2),
                TransformFromCopy(sqrt_result, answer2_exact, run_time=2)
            )
            
        with self.voiceover(
            text="""Which equals approximately negative four. point eight five four."""
        ) as tracker:
            self.play(Write(answer1_decimal, run_time=2))
            
        with self.voiceover(
            text="""For the second solution, we subtract: 
            negative three minus three times the square root of five, divided by two. 
            Which equals approximately one point eight five four."""
        ) as tracker:
