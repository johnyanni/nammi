...
class QuadraticFormula03a(MathTutorialScene):
    def construct(self):
        ...
        with self.voiceover(
            text=f"""We subtract 4x from both sides to get x squared {B_VALUE:+d}x {C_VALUE:+d} equals 0.
            <bookmark mark="A"/>
        ...
        with self.voiceover(
            text=f"""The coefficient of x is {B_VALUE}."""
        ) as tracker:
        ...
        with self.voiceover(
            text=f"""And the constant term is {C_VALUE}."""
        ) as tracker:
        ...
        with self.voiceover(
            text=f"""We replace b with {B_VALUE}, appearing twice in the formula."""
        ) as tracker:
        ...
        # Handle -b calculation with conditional voiceover
        if B_VALUE < 0:
            neg_b_text = f"""We have negative {B_VALUE}, which becomes positive {NEG_B}."""
        else:
            neg_b_text = f"""We have negative {B_VALUE}, which is {NEG_B}."""
        ...
        # Calculate discriminant step
        if FOUR_AC < 0:
            discriminant_text = f"""{B_SQUARED} minus {FOUR_AC} is {B_SQUARED} plus {-FOUR_AC}, which equals {DISCRIMINANT}."""
        else:
            discriminant_text = f"""{B_SQUARED} minus {FOUR_AC} equals {DISCRIMINANT}."""
            
        with self.voiceover(
            text=discriminant_text
        ) as tracker:
        ...
        with self.voiceover(
            text=f"""Now we need to simplify the square root of {DISCRIMINANT}."""
        ) as tracker:
            scroll.prepare_next(run_time=2)  # Show label
            self.play(self.indicate(simplify2_sqrt, run_time=2))
            
        with self.voiceover(
            text=f"""We can factor {DISCRIMINANT} as 4 times 7."""
        ) as tracker:
        ...
        with self.voiceover(
            text=f"""We can simplify further by dividing both terms in the numerator by {TWO_A}."""
        ) as tracker:
            scroll.prepare_next(run_time=2)  # Show label
            scroll.prepare_next(run_time=4)  # Show division steps
        
        with self.voiceover(
            text=f"""{NEG_B}. divided by {TWO_A} is {NEG_B//TWO_A}."""
        ) as tracker:
        ...
        with self.voiceover(
            text=f"""And 2 square root 7 divided by {TWO_A} is square root 7."""
        ) as tracker:
            scroll.transform_from_copy(
        ...
        with self.voiceover(
            text=f"""So our solution is x equals {NEG_B//TWO_A} plus or minus square root 7."""
        ) as tracker:
            self.wait(1)
        ...
        with self.voiceover(
            ...
            {NEG_B//TWO_A} minus square root 7."""
        ) as tracker:
            self.play(
        ...
        with self.voiceover(
            ...
            {NEG_B//TWO_A} plus square root 7, which equals approximately {SOLUTION2_DECIMAL:.3f}."""
        ) as tracker:
