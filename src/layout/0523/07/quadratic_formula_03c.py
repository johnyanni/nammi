...
class QuadraticFormula03a(MathTutorialScene):
    def construct(self):
        ...
        rearrange_step1 = MathTex(rf"x^2 {B_VALUE:+d}x {C_VALUE:+d} = 0").scale(MATH_SCALE)
        ...
        identify_equation = MathTex(rf"x^2 {B_VALUE:+d}x {C_VALUE:+d} = 0").scale(MATH_SCALE)
        ...
        simplify2_equation = MathTex(
            rf"x = \frac{{{NEG_B} \pm \sqrt{{{DISCRIMINANT}}}}}{{2}}"
        ).scale(MATH_SCALE)
        ...
        sqrt_result = MathTex(rf"x = \frac{{{NEG_B} \pm 2\sqrt{{7}}}}{{2}}").scale(MATH_SCALE)
        ...
        # ============================================
        final_label = Tex(r"Simplify further by dividing out common factors").scale(LABEL_SCALE)
        final_equation = MathTex(
            rf"x = \frac{{{NEG_B} \pm 2\sqrt{{7}}}}{{2}} = \frac{{{NEG_B}}}{{2}} \pm \frac{{2\sqrt{{7}}}}{{2}}"
        ).scale(MATH_SCALE)
        ...
        final_parsing = self.parse_elements(final_equation,
            ('first_frac', rf'\frac{{{NEG_B}}}{{2}}', 0, LIGHT_GRAY),
        ...
        with self.voiceover(
            text="""Let's solve this equation using the quadratic formula. 
            We have x minus three over x equals four."""
        ) as tracker:
        ...
        with self.voiceover(
            text="""On the left side, x times x is x squared, and x times negative three over x is negative three.
            On the right side, four times x is four x. This gives us x squared minus three equals four x."""
        ) as tracker:
        ...
        with self.voiceover(
            text="""We subtract four x from both sides to get x squared minus four x minus three equals zero.
            <bookmark mark="A"/>
        ...
        with self.voiceover(
            text="""In our equation, the coefficient of x squared is one."""
        ) as tracker:
        ...
        with self.voiceover(
            text="""The coefficient of x is negative four."""
        ) as tracker:
        ...
        with self.voiceover(
            text="""And the constant term is negative three."""
        ) as tracker:
        ...
        with self.voiceover(
            text="""We replace b with negative four, appearing twice in the formula."""
        ) as tracker:
        ...
        with self.voiceover(
            text="""We replace a with one in the four ac term."""
        ) as tracker:
        ...
        with self.voiceover(
            text="""And a with one again in the denominator."""
        ) as tracker:
            self.play(self.indicate(coefficient_values['a_value'], color=None, run_time=2))
        ...
        with self.voiceover(
            text="""We have negative negative four, which becomes positive four.
            Negative four squared is sixteen. 
            Four times one times negative three is negative twelve. 
        ...
        with self.voiceover(
            text="""Sixteen minus negative twelve is sixteen plus twelve, which equals twenty-eight."""
        ) as tracker:
        ...
        with self.voiceover(
            text="""Now we need to simplify the square root of twenty-eight."""
        ) as tracker:
            scroll.prepare_next(run_time=2)  # Show label
            self.play(self.indicate(simplify2_sqrt, run_time=2))
            
        with self.voiceover(
            text="""We can factor twenty-eight as four times seven."""
        ) as tracker:
        ...
        with self.voiceover(
            text="""Since the square root of four is two, 
            we get two times the square root of seven."""
        ) as tracker:
        ...
        with self.voiceover(
            text="""We can easily simplify further by splitting up the fraction and dividing out common factors."""
        ) as tracker:
            scroll.prepare_next(run_time=2)  # Show label
            scroll.prepare_next(run_time=4)  # Show division steps
        
        with self.voiceover(
            text="""On the left side of the fraction, four. divided by two is two."""
        ) as tracker:
        ...
        with self.voiceover(
            text="""On the right, we can reduce this fraction by canceling the common factor of 2. Leaving us with the square root of seven."""
        ) as tracker:
            scroll.transform_from_copy(
        ...
        with self.voiceover(
            text="""So our solution is x equals two plus or minus the square root of seven."""
        ) as tracker:
            self.wait(1)
        ...
        with self.voiceover(
            ...
            two minus the square root of seven."""
        ) as tracker:
            self.play(
        ...
        with self.voiceover(
            text="""Which equals approximately negative zero point six four six."""
        ) as tracker:
            self.play(Write(answer1_decimal, run_time=2))
            
        with self.voiceover(
            text="""For the second solution, we add: 
            two plus the square root of seven, which equals approximately four point six four six."""
        ) as tracker:
