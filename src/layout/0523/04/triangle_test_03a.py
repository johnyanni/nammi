...
class SimpleTriangle:
    """Simplified Triangle class for visualization only (no solution generation)."""
    ...
    def get_normal(self, line_start_pt, line_end_pt, proportion=0.5, length=1.0, angle=90*DEGREES):
    ...
    def curved_arrow_to_angle(self, mob, angle_mob, angle=90*DEGREES, proportion=0.5, 
                             length=None, buff=0.5, start_direction='bottom', **kwargs):
        ...
        l = Line(line_start_pt, line_end_pt)
        normal = self.get_normal(line_start_pt, line_end_pt, proportion, length, angle)
    ...
    def _build(self):
        ...
        if not self.h:
            ...
            label_h_position = self.get_normal(D, A, length=self.label_shift)
        ...
        if self.a:
            label_a_position = self.get_normal(A, C, length=self.label_shift)
        ...
        if self.b:
            label_b_position = self.get_normal(B, A, length=self.label_shift)
        ...
        if self.c:
            ...
            label_c_position = self.get_normal(C, B, length=self.label_shift + label_shift_offset)
            ...
            label_c_name_position = self.get_normal(C, B, length=self.label_shift + label_shift_offset - 0.5)
        ...
        if self.alpha:
            ...
            if not self.indicate_alpha_with_arrow:
                alpha_position = self.get_normal(alpha_obj.get_end(), alpha_obj.get_start(), length=self.angle_shift)
        ...
        if self.beta:
            ...
            if not self.indicate_beta_with_arrow:
                beta_position = self.get_normal(beta_obj.get_end(), beta_obj.get_start(), length=self.angle_shift)
        ...
        if self.h:
            self.components["triangle_w_h"] = VGroup(triangle, altitude)
        ...
        if self.alpha:
            ...
            if self.indicate_alpha_with_arrow:
                self.components["angle_value"].add(alpha_arrow)
        elif self.beta:
            self.components["angle_value"] = VGroup(beta_obj, beta_label)
            if self.indicate_beta_with_arrow:
                self.components["angle_value"].add(beta_arrow)
...
class TriangleDemo(Scene):
    def construct(self):
        ...
        t1 = SimpleTriangle(
            ...
            right_angle_position="bottom_left",
            label_scale=0.5
        ).triangle().scale(0.7).shift(LEFT * 3 + UP * 2)
        ...
        t2 = SimpleTriangle(
            ...
            label_scale=0.5
        ).triangle().scale(0.5).shift(RIGHT * 3 + UP * 2)
        ...
        t3 = SimpleTriangle(
            ...
            indicate_alpha_with_arrow=True,
            indicate_beta_with_arrow=True
        ).triangle().scale(0.6).shift(LEFT * 3 + DOWN * 2)
        ...
        t4 = SimpleTriangle(
            ...
            label_scale=0.5
        ).triangle().scale(0.4).shift(RIGHT * 3 + DOWN * 2)
        
        self.add(t1, t2, t3, t4)
        
        # Add labels
        self.add(Text("3-4-5 Triangle", font_size=16).next_to(t1, UP))
        self.add(Text("5-12-13 Triangle", font_size=16).next_to(t2, UP))
        self.add(Text("With Arrows", font_size=16).next_to(t3, UP))
