...
class Fraction(MathTutorialScene):
    def construct(self):
        ...
        VGroup(equation, sol1).arrange(DOWN, aligned_edge=RIGHT, buff=.8)
        sol2.next_to(sol1, RIGHT)
        little_10 = MathTex("10", color=RED).scale(.6).next_to(equation[0][0], LEFT, buff=.2)
        little_3 = MathTex("3", color=RED).scale(.6).next_to(equation[0][4], LEFT, buff=.2)
        ...
        self.play(Write(
            VGroup(sol1[0][2], sol1[0][4], sol1[0][-1])
        ))
        self.play(
            AnimationGroup(
