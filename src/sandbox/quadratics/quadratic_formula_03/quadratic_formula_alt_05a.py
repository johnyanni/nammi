...
class QuadraticFormula(MathTutorialScene):
    def construct(self):
        ...
        initial_label = Tex("First, we need to transform to standard form").scale(TEXT_SCALE)
        ...
        div_4 = self.add_annotations(
            r"\div 4", 
            self.find_element("4", initial_exp),
            self.find_element("48", initial_exp),
            color=BLUE
        )

        subtract_12 = self.add_annotations(
            "-12", 
            self.find_element("25", expanded_exp),
            self.find_element("12", expanded_exp),
            color=RED
        )

        # 3. Create the math steps - let's modify how we access them later
        step1_title = Tex("Step 1: First, we need to get the equation in standard form").scale(TEXT_SCALE)
        step2_title = Tex("Step 2: Divide both sides by 4").scale(TEXT_SCALE)
        step3_title = Tex("Step 3: Expand the squared term").scale(TEXT_SCALE)
        step4_title = Tex("Step 4: Subtract 12 from both sides").scale(TEXT_SCALE)

        # Organize them with expressions
        step1_group = VGroup(step1_title, initial_exp.copy()).arrange(DOWN, aligned_edge=LEFT, buff=0.2)
        step2_group = VGroup(step2_title, initial_exp.copy(), div_4, divided_exp.copy()).arrange(DOWN, aligned_edge=LEFT, buff=0.2)
        step3_group = VGroup(step3_title, expanded_exp.copy()).arrange(DOWN, aligned_edge=LEFT, buff=0.2)
        step4_group = VGroup(step4_title, expanded_exp.copy(), subtract_12, final_exp.copy()).arrange(DOWN, aligned_edge=LEFT, buff=0.2)
        ...
        pre_solution_steps = VGroup(
            ...
            step4_group,
            final_exp
        ).arrange(DOWN, aligned_edge=LEFT, buff=0.1).next_to(question_title_group, DOWN, buff=0.5).align_to(question_title_group, LEFT)

        pre_solution = pre_solution_steps
        ...
        self.play(Write(step1_group[1]))  # The expression

        # Step 2: Division step with annotation
        self.play(Write(step2_title))
        self.play(Write(step2_group[1]))  # First expression
        self.play(FadeIn(div_4))
        self.play(Write(step2_group[3]))  # Second expression

        # Step 3: Expansion step
        self.play(Write(step3_title))
        self.play(Write(step3_group[1]))  # The expression

        # Step 4: Subtraction step with annotation
        self.play(Write(step4_title))
        self.play(Write(step4_group[1]))  # First expression
