...
class QuadraticFormula(MathTutorialScene):
    def construct(self):
        ...
        # Integration with the main code
        ...
        initial_exp = MathTex("4(x+5)^2=48").scale(TEX_SCALE)
        expanded_exp = MathTex("x^2 + 10x + 25 = 12").scale(TEX_SCALE)
        final_exp = MathTex("x^2 + 10x + 13 = 0").scale(TEX_SCALE)
        divided_exp = MathTex("(x+5)^2=12").scale(TEX_SCALE)

        # 2. Create annotations using the original expressions
        div_4 = self.add_annotations(
            "÷4", 
            self.find_element("4", initial_exp, as_group=True),
            self.find_element("48", initial_exp, as_group=True),
            color=BLUE
        )

        subtract_12 = self.add_annotations(
            "-12", 
            self.find_element("25", expanded_exp, as_group=True),
            self.find_element("12", expanded_exp, as_group=True),
            color=RED,
            h_spacing=0.3
        )

        # 3. Create the math steps using copies of the original expressions
        # Use a single text label for safety, with both parts in one string
        pre_sol_step_0 = self.create_math_step_with_inline_annotations(
            step_number=1,
            text_labels="First, we need to get the equation in standard form",
            expressions=initial_exp.copy()
        )

        pre_sol_step_1 = self.create_math_step_with_inline_annotations(
            step_number=2,
            text_labels="Divide both sides by 4",
            expressions=[initial_exp.copy(), divided_exp],
            annotations=div_4,
            annotation_placements=0
        )

        pre_sol_step_2 = self.create_math_step_with_inline_annotations(
            step_number=3,
            text_labels="Expand the squared term",
            expressions=expanded_exp.copy()
        )

        pre_sol_step_3 = self.create_math_step_with_inline_annotations(
            step_number=4,
            text_labels="Subtract 12 from both sides",
            expressions=[expanded_exp.copy(), final_exp],
            annotations=subtract_12,
            annotation_placements=0
        )

        # 4. Arrange steps and create group
        ...
        pre_solution = VGroup(pre_sol_step_0, pre_sol_step_1, pre_sol_step_2, pre_sol_step_3)
        final_equation = final_exp
        ...
        self.play(Write(pre_sol_step_0[0][0][0]))  # First text label
        self.play(Write(pre_sol_step_0[0][0][1]))  # Second text label
        self.play(Write(pre_sol_step_0[0][1][0]))  # First expression

        self.play(Write(pre_sol_step_1[0][0]))     # Title
        self.play(Write(pre_sol_step_1[0][1][0]))  # First expression
        # Division annotation appears automatically with the first expression
        self.play(Write(pre_sol_step_1[0][1][2]))  # Second expression (after annotation)

        self.play(Write(pre_sol_step_2[0][0]))     # Title
        self.play(Write(pre_sol_step_2[0][1][0]))  # Expression

        self.play(Write(pre_sol_step_3[0][0]))     # Title
        self.play(Write(pre_sol_step_3[0][1][0]))  # First expression
        # Subtraction annotation appears automatically with the first expression
        self.play(Write(pre_sol_step_3[0][1][2]))  # Final equation (after annotation)
