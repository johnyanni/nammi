...
class QuadraticFormula(MathTutorialScene):
    def construct(self):
        ...
        step_1_tex = r"x = 5 + 3 \frac{-(11) \pm \sqrt{(11)^2 - 4(1)(20)}}{2(1)}"
        ...
        # Find basic elements
        step_1_fraction = step_1_exp[0][27]  # Known index for fraction
        step_1_x_part = find_element("x = 5 + 3")
        step_1_plus_minus_part = find_element(r"\pm")
        step_1_sqrt_part = VGroup(step_1_exp[0][11:13])  # Known indices for sqrt
        ...
        # MathIndices.display_indices(self, step_1_exp, "step_1_exp")
        
        # Solution Step 2 - Simplified
        ...
        # Only get the parts we need to highlight in step 2
        ...
        # Only get the part we need to highlight in step 3
        ...
        with self.voiceover(
            ...
            # Show the step 2 label first
            self.play(Write(step_2_label))
            
            # Simply fade in the step 2 equation
            self.play(FadeIn(step_2_exp))
            
            # Highlight specific parts at each bookmark
            self.wait_until_bookmark("b_squared")
            self.play(self.indicate(step_2_exp[0][step_2_121_idx]))
                
            self.wait_until_bookmark("four_ac")
            self.play(self.indicate(step_2_exp[0][step_2_80_idx]))
            
            self.wait_until_bookmark("den")
            self.play(self.indicate(step_2_exp[0][step_2_2_idx]))

            self.wait(COMPREHENSION_PAUSE)
            
        # SIMPLIFIED STEP 3 ANIMATION
        with self.voiceover(
                text="""
                And finally, 121 minus 80 <bookmark mark='equation' /> gives us 41.
                """
        ) as tracker:
            # Show the step 3 label first
            self.play(Write(step_3_label))
            
            # Simply fade in the step 3 equation
            self.play(FadeIn(step_3_exp))
            
            # Highlight the 41 at the bookmark
            self.wait_until_bookmark("equation")
