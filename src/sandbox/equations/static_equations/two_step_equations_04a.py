from manim import *
from src.components.common.base_scene import *
from src.components.common.quick_tip import QuickTip




# Colors
VARIABLE_COLOR  = "#BB88FF"
COEFF_COLOR     = "#FF8C42"
CONSTANT_COLOR  = "#43D9A2"
TITLE_COLOR     = "#FFFFFF"
PLUS_COLOR      = "#3DCFF4"
MINUS_COLOR     = "#FF5C57"
TIMES_COLOR     = "#D77BFF"
DIVISION_COLOR  = "#FF9AC1"
ARROW_COLOR     = "#A0A0A0"

class SolveForX(MathTutorialScene):
    def construct(self):
        # Constants
        EQUATION_SPACING = 1.3
        
        # Title and Operations
        title = Tex("Solve for x", color=TITLE_COLOR).to_corner(UL)
        
        # Create equations and manually set colors
        equation = MathTex(r"\frac{x}{7} + 3 = -2").scale(TEX_SCALE)
        step_1 = MathTex(r"\frac{x}{7} = -5").scale(TEX_SCALE)
        solution = MathTex("x = -35").scale(TEX_SCALE)
        
        # Apply colors using the existing smart colorize method from the parent class
        self.apply_smart_colorize(
            [equation, step_1, solution],
            {
                "7": COEFF_COLOR,
                "x": VARIABLE_COLOR,
                "3": CONSTANT_COLOR,  
                "2": CONSTANT_COLOR, 
                "5": CONSTANT_COLOR,
                "35": CONSTANT_COLOR,
                "-": MINUS_COLOR,
                "+": PLUS_COLOR,
            }
        )
        
        # Arrange and position equations
        equation_group = VGroup(equation, step_1, solution).arrange(DOWN, buff=EQUATION_SPACING)
        equation_group.to_edge(UP, buff=1)
        
        # Create solution box
        solution_rec = SurroundingRectangle(solution, buff=0.2, color=WHITE)
        
        # Find elements for annotations
        plus_3 = self.find_element("3", equation)
        minus_2 = self.find_element("2", equation)
        subtract_3 = self.add_annotations("-3", plus_3, minus_2, color=CONSTANT_COLOR)
        
        # Annotations for step 2
        step_1_x_over_7 = self.find_element(r"\frac{x}{7}", step_1)
        step_1_minus_5 = self.find_element("5", step_1)
        multiply_7 = self.add_annotations(r"\times 7", step_1_x_over_7, step_1_minus_5, color=COEFF_COLOR)
        
        # Tips
        tip_1 = QuickTip(
            "Get rid of plus/minus before multiply/divide",
            color_map={
                "plus": PLUS_COLOR,
                "minus": MINUS_COLOR,
                "multiply": TIMES_COLOR,
                "divide": DIVISION_COLOR
            }
        ).to_corner(DR)
        
        tip_2 = QuickTip(
            r"\textbf{Step 1}: Subtract 3 from both sides to eliminate +3",
            color_map={"3": CONSTANT_COLOR}
        ).to_corner(DR)
        
        tip_3 = QuickTip(
            r"\textbf{Step 2}: Multiply both sides by 7 to isolate x",
            color_map={"7": COEFF_COLOR, "x": VARIABLE_COLOR}
        ).to_corner(DL)
        
        # Animation with voiceover
        with self.voiceover(
            text="""
            Let's solve <bookmark mark='equation' /> this equation for x.
            We have x over 7 plus 3 equals negative 2.
            
            To solve for x, we need to get it by itself on one side of the equation.
            We'll do this in two steps.
            """
        ) as tracker:
            self.play(FadeIn(title))
            self.wait_until_bookmark("equation")
            self.play(Write(equation))
        
        # Pause after introducing the equation
        self.wait(2)
        
        with self.voiceover(
            text="""
            When solving equations, it's important to remember that we should handle addition and subtraction before multiplication and division.
            <bookmark mark='tip_in' /> This helps us work through the problem systematically.
            """
        ) as tracker:
            self.wait_until_bookmark("tip_in")
            self.play(FadeIn(tip_1, shift=UP))
        
        # Pause to let students read the tip
        self.wait(3)
        self.play(FadeOut(tip_1, shift=DOWN))
        
        with self.voiceover(
            text="""
            Step 1: We need to eliminate the positive 3 on the left side.
            <bookmark mark='tip_in' /> To do this, we'll subtract 3 from both sides of the equation.
            
            <bookmark mark='subtract' /> 3 minus 3 gives us 0, which cancels out.
            On the right side, negative 2 minus 3 equals negative 5.
            
            <bookmark mark='result' /> So now we have x over 7 equals negative 5.
            """
        ) as tracker:
            self.wait_until_bookmark("tip_in")
            self.play(FadeIn(tip_2, shift=UP))
            
            self.wait_until_bookmark("subtract")
            self.play(FadeIn(subtract_3), run_time=1.5)
            
            self.wait_until_bookmark("result")
            self.play(
                FadeIn(step_1), 
                FadeOut(tip_2, shift=DOWN),
                run_time=2
            )
        
        # Comprehension pause after completing step 1
        self.wait(2)
        
        with self.voiceover(
            text="""
            Step 2: Now we need to isolate x.
            <bookmark mark='tip_in' /> Since x is being divided by 7, we'll multiply both sides by 7.
            
            <bookmark mark='multiply' /> x over 7 times 7 gives us just x.
            And negative 5 times 7 equals negative 35.
            
            <bookmark mark='solution' /> Therefore, x equals negative 35.
            """
        ) as tracker:
            self.wait_until_bookmark("tip_in")
            self.play(FadeIn(tip_3, shift=UP))
            
            self.wait_until_bookmark("multiply")
            self.play(FadeIn(multiply_7), run_time=1.5)
            
            self.wait_until_bookmark("solution")
            self.play(
                FadeIn(solution), 
                FadeOut(tip_3, shift=DOWN),
                run_time=2
            )
        
        # Quick pause before verification
        self.wait(1.5)
        
        with self.voiceover(
            text="""
            Let's verify our answer. 
            If x equals negative 35, then negative 35 divided by 7 is negative 5, plus 3 gives us negative 2.
            That matches the original equation, so our solution is correct!
            
            <bookmark mark='highlight' /> x equals negative 35 is our final answer.
            """
        ) as tracker:
            self.wait_until_bookmark("highlight")
            self.play(Create(solution_rec), run_time=1.5)
        
        # Final pause at the end
        self.wait(3)